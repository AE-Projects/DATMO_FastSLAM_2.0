...
mov_obj_motion_type = 2; % 1 for line and 2 for circle;
...
if mov_obj_motion_type ==1
    simOutFile = 'so_sym2_nk_wmov.txt';
    mapFile = 'map_sym2.txt';
    load('mov_obj_pos.mat');
else
    %simOutFile = 'so_sym2_nk_wmov_circ6.txt';
    simOutFile = 'so_sym2_nk_wmov_2_land_circ1.txt';
    mapFile = 'map_sym2.txt';
    load('mov_obj_circ_pos6.mat');
end
...
P_t = [0.05 0 0; 0 0.05 0; 0 0 deg2rad(2)];
S_t = [0.05^2 0; 0 deg2rad(5)^2];
...
for m = 1:M
    %X = rand(1) * (max(map(1, :)) - min(map(1, :)) + 2*particle_margin) + min(map(1, :)) - particle_margin;
    %Y = rand(1) * (max(map(2, :)) - min(map(2, :)) + 2*particle_margin) + min(map(2, :)) - particle_margin;
...
for t = 2:n_timesteps
    ...
    if ~isempty(mov_idx)
        ...
        z_m(:,t) = z_t(1:2, mov_idx);
    end
    z_t_m = z_t(1:2, mov_idx);  
    z_t_s = z_t(1:2,:);
    z_t_s(:,mov_idx) = [];

    [Y_t] = fast_slam_func(z_t_s, u_t, delta_t, Y_t_1, R, Q);
    Y_t_1 = Y_t;
    % get pose estimate
    accm_pose = cat(2,Y_t_1.pose);
    fast_slam_pose(:,:,t) = accm_pose;
    mu_t_rob = mean(accm_pose, 2);
    mu_t_rob(3) = atan2(mean(sin(accm_pose(3, :))), mean(cos(accm_pose(3, :))));
    pos_sigma = cov(accm_pose(1,:),accm_pose(2,:));
    var_theta = var(sin(accm_pose(3, :))) + var(cos(accm_pose(3, :)));
    sigma_t_rob = zeros(3,3);
    sigma_t_rob(1:2, 1:2) = pos_sigma;
    sigma_t_rob(3, 3) = var_theta;
    mu_rob_arr(:,t) = mu_t_rob;
    %Define z_t_m
    %[mu_t, sigma_t, N_t] = ekf_slam_datmo_func(mu_t_1, sigma_t_1, u_t, z_t_s, N_t_1,delta_t, R, Q);
    %mu_t_rob = mu_t(1:3,1);
    %sigma_t_rob = sigma_t(1:3, 1:3);
    if first_detect== mov_obj_motion_type+2
        if mov_obj_motion_type ==1 
            mu_mov_t_1 = measurement_model_inverse(z_m(:,t-1), slam_pose(:,t-1));
            mu_mov_t = measurement_model_inverse(z_m(:,t), mu_t_rob);
            theta1 = atan((mu_mov_t(2,1) - mu_mov_t_1(2,1))/(mu_mov_t(1,1) - mu_mov_t_1(1,1)));
            v0 = (sqrt((mu_mov_t(2,1) - mu_mov_t_1(2,1))^2 +(mu_mov_t(1,1) - mu_mov_t_1(1,1))^2))/delta_t;
            %v0 = 0.15;
            %theta1 = 1.2;
            w0 = 0;
        else 
            mu_mov_t_2 = measurement_model_inverse(z_m(:,t-2), slam_pose(:,t-2));
            mu_mov_t_1 = measurement_model_inverse(z_m(:,t-1), slam_pose(:,t-1));
            mu_mov_t = measurement_model_inverse(z_m(:,t), mu_t_rob);
            theta0 = -atan((mu_mov_t_1(2,1) - mu_mov_t_2(2,1))/(mu_mov_t_1(1,1) - mu_mov_t_2(1,1)));
            theta1 = -atan((mu_mov_t(2,1) - mu_mov_t_1(2,1))/(mu_mov_t(1,1) - mu_mov_t_1(1,1)));
            w00 = -(theta1-theta0); % 2*delta_t or delta_t or else
            w0 = 0.0255;
            v0 = (sqrt((mu_mov_t(2,1) - mu_mov_t_1(2,1))^2 +(mu_mov_t(1,1) - mu_mov_t_1(1,1))^2))/delta_t;
            %v0 = 0.25;
        end
        mu_0_m = mu_t_rob + [z_t_m(1,1)*cos(z_t_m(2,1)); z_t_m(1,1)*sin(z_t_m(2,1)); theta1];
        sigma_0_m = 10^-1*eye(3,3);
        [mu_t_m, sigma_t_m] = datmo_func(mu_0_m, sigma_0_m, mu_t_rob, sigma_t_rob, z_t_m, P_t, S_t, delta_t,v0,w0);
        % first_detect = 2;
        fprintf('First time detected')
    elseif first_detect>mov_obj_motion_type+1
        disp(v0);
        [mu_t_m, sigma_t_m] = datmo_func(mu_t_1_m, sigma_t_1_m, mu_t_rob, sigma_t_rob, z_t_m, P_t, S_t, delta_t,v0,w0);
        gm0(:,t) = [mu_t_1_m;mu_t_m];
    end
    ...
    if first_detect<=mov_obj_motion_type+1
        plot_fastSLAM_DATMO(map, simSteps{t}.truePose, z_t_s, z_t_m, [xm(1,t);ym(1,t)], Y_t_1, fast_slam_pose, 0.01*eye(3,3), 0, color)
        p = p+1;
    else
        %mu_t_m = zeros(3,1);
        %sigma_t_m = zeros(3,3)
        ...
        plot_fastSLAM_DATMO(map, simSteps{t}.truePose, z_t_s, z_t_m, [xm(1,t);ym(1,t)], Y_t_1, fast_slam_pose, sigma_t_m, mu_t_m, color)
    end
    drawnow;
    hold on
end
...
plot(mov_path_pred(1,p+1:end), mov_path_pred(2,p+1:end), 'LineWidth', 1.5)
